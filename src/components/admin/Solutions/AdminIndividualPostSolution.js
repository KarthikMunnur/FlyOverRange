...
import ModerateSolutions from "./ModerateSolutions";
import ModerateApprovedSolution from "./ModerateApprovedSolution";
import ModerateRejectedSolutions from "./ModerateRejectedSolutions";

class AdminIndividualPostSolution extends Component {
  constructor(props) {
    super(props);
    this.state = {
      solutionsFilter: [],
  ...
  componentDidMount() {
    ...
    if (window.location.pathname === "/yet-Moderate") {
      db.collection("moderatesolutions")
        .where("postid", "==", this.props.props.id)
        .get()
        .then((querySnapshot) => {
          const data = querySnapshot.docs.map((doc) => doc.data());
          this.setState({ solutionsFilter: data });
        })
        .catch((err) => {
          console.log(err);
        });
    } else if (window.location.pathname === "/acceptedpost") {
      db.collection("approvedsolution")
      ...
      db.collection("rejectedsolution")
        .where("postid", "==", this.props.props.postid)
        .get()
        .then((querySnapshot) => {
          const data = querySnapshot.docs.map((doc) => doc.data());
  ...
  render() {
    ...
    if (this.state.solutionsFilter.length > 0) {
      return (
        <div className="cards w-100">
          ...
          <div className="w-100">
            {(() => {
              if (window.location.pathname === "/yet-Moderate") {
                return <ModerateSolutions key={post.id} props={post} />;
              } else if (window.location.pathname === "/acceptedpost") {
                return <ModerateApprovedSolution key={post.id} props={post} />;
              } else if (window.location.pathname === "/rejectedpost") {
                return <ModerateRejectedSolutions key={post.id} props={post} />;
              }
            })()}
          </div>
        </div>
...
export default connect(mapStateToProps)(AdminIndividualPostSolution);
